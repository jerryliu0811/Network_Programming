				...
				for(iter = clientTable.begin(); iter != clientTable.end(); iter++){
					if(iter->second.fd != serversock){
						if(strcmp(dest_clientname, "no name") == 0){
							sprintf(buffer, "*** %s (#%d) just piped '%s' to (%s) (#%d) ***\n", myname, myid, line_cpy, clientTable[dest_clientfd].name, dest_clientId);
						}
						else{
							sprintf(buffer, "*** %s (#%d) just piped '%s' to %s (#%d) ***\n", myname, myid, line_cpy, clientTable[dest_clientfd].name, dest_clientId);
						}
...
void initClient(int clientsock, char *clientIP, int clientPORT){
...
int main(int argc, char *argv[]){
	...
	while(1){
		...
		if(FD_ISSET(serversock, &rfds)){
			...
			initClient(clientsock, clientIP, clientPORT);
